<template>
  <el-card shadow="never" class="card-box">
    ...
    <div v-auth="'type:ticket:add'" class="table-btn-box mb10">
    ...
    <el-table v-loading="tableLoading.status" :data="tableData.data" border row-key="id" @sort-change="sortChange">
      ...
      <el-table-column prop="name" label="类型名称" align="center" show-overflow-tooltip/>
      <el-table-column label="操作" fixed="right" align="center" min-width="116">
        <template #default="scope">
          <el-button v-auth="'type:ticket:update'" link type="primary" @click="openDialog(scope.row.id)">
            <el-icon>
              <ele-edit/>
            </el-icon>
            修改
          </el-button>
          <el-button v-auth="'type:ticket:delete'" link type="primary" @click="delTable(scope.row.id)">
            <el-icon>
              <ele-delete/>
            </el-icon>
            删除
...
import {deleteTypeTicket, getTypeTicketPage} from "@/api/test/typeTicket";
...
const getTableList = () => {
    ...
    getTypeTicketPage({...pagination, ...queryForm.value, ...orderBy.value}).then(res => {
